...
export function ExperienceSection({ dictionary }: ExperienceSectionProps) {
    return (
        ...
        >
            <h1 id="experience" className="pt-20 md:pt-32 max-w-5xl font-bold text-2xl text-[#0b1d3a] md:text-7xl dark:text-slate-100">
                {dictionary.title}
            </h1>
            <div className="relative mx-auto pt-4 pb-32 max-w-2xl text-[#0b1d3a] antialiased dark:text-slate-200">
                {dictionary.items.map((item, index) => (
                    <div key={index}>
                        <div key={`content-${index}`} className="mt-4 md:mt-0 mb-10">
                            <h2 className={cn('text-xl text-[#0b1d3a] dark:text-slate-100')}>{item.title}</h2>
                            <span className="py-1 rounded-full w-fit text-primary/60 dark:text-primary/80 text-sm italic">{item.badge}</span>
